      ...
      wire.on('extended', function (id, ext) {
        ...
        try {
          ...
          message = bncode.decode(
            ext.slice(0, delimiter === -1 ? ext.length : delimiter + 2)
          )
          piece = message.piece
        } catch (err) {
          return
        ...
        if (message.msg_type === 0) {
          if (!metadata)
            return wire.extended(channel, { msg_type: 2, piece: piece })
          var offset = piece * METADATA_BLOCK_SIZE
          var buf = metadata.slice(offset, offset + METADATA_BLOCK_SIZE)
          wire.extended(
            channel,
            Buffer.concat([bncode.encode({ msg_type: 1, piece: piece }), buf])
          )
          return
        }
